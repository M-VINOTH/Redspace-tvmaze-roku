...
function TransactionNetwork_makeBulkRequest(requests as object, callback as dynamic, id = "" as string) as dynamic
	context = invalid
	if isNonEmptyString(callback) or type(callback) = "roMessagePort" and isNonEmptyArray(requests)
		context = createNode("Node", {"response": []})
		if isNonEmptyString(id) then context.id = id
		context.observeFieldScoped("response", callback)
		getTransactionNetworkService().request = {
			"type": "bulk"
			"context": context
			"requests": requests
		}
...
function TransactionNetwork_makeRequest(endpointPath as string, callback = invalid as dynamic, parameters = {} as object, id = "" as string) as object
	if LCase(Left(endpointPath, 4)) = "http"
		endpoint = {"url": endpointPath}
		if isNonEmptyString(parameters.method)
			endpoint.method = parameters.method
		end if
	end if

	if (not isAA(parameters))
		return invalid
	end if
	...
	requestHeaders = {}
	accessToken = getValueAtKeyPath(parameters, "accessToken", "")
	if isNonEmptyString(accessToken)
		requestHeaders["Authorization"] = "Bearer " + accessToken
	end if

	' Appending content-type header if the post body is array or assoc array or request will fail.
	if (isAA(parameters.body) or isArr(parameters.body))
		requestHeaders["Content-Type"] = "application/json"
	else
		requestHeaders["Content-Length"] = "0"
	end if

	if isNonEmptyAA(parameters.headers)
		requestHeaders.append(parameters.headers)
	end if

	parameters.append(requestParameters)
	parameters.headers = requestHeaders

	'Removing additionalData and parserConfiguration from the request if it's not an AA
	if (not isAA(parameters.additionalData))
		parameters.delete("additionalData")
	end if
	if (not isAA(parameters.parserConfiguration))
		parameters.delete("parserConfiguration")
	end if

	parameters.maxRetries = getValueAtKeyPath(parameters, "maxRetries", 0)
	parameters.maxRetryTimeout = getValueAtKeyPath(parameters, "maxRetryTimeout", 0)

	context = createNode("Node", {"response": {"subType": "Node"}, "preResponse": {"subType": "Node"}, "kill": false, "requestIdentifier": getDeviceInfo().getRandomUUID()})
	if isNonEmptyString(id) then context.id = id
	...
	if (isNonEmptyString(callback) or Type(callback) = "roMessagePort" or isInvalid(callback))
		if isInvalid(callback)
			request.processResponse = false
		else
			context.observeFieldScoped("response", callback)
			context.observeFieldScoped("preResponse", "Network_PreResponseCheckForErrors")
		end if

		transactionNetworkService = getTransactionNetworkService()
		request.type = "single"
		transactionNetworkService.request = request
...
sub Network_PreResponseCheckForErrors(event as object)
	response = event.getData()
	'TODO check for server error response
	event.getRoSGNode().response = response
end sub

function isResponseCodeSuccess(code = 0 as integer) as boolean
	HTTP_CODE_RANGE = {
		SUCCESS_FROM: 200,
		SUCCESS_TO: 399,
	}
	return (code >= HTTP_CODE_RANGE.SUCCESS_FROM and code <= HTTP_CODE_RANGE.SUCCESS_TO)
end function

function isResponseCodeExpiredTokenRocket(code = 0 as integer) as boolean
	return (code = 401)
end function

function isResponseCodeExpiredTokenBellUM(code = 0 as integer) as boolean
	return (code = 401 or code = 403)
end function

'https://developer.roku.com/en-gb/docs/references/brightscript/events/rourlevent.md
' 0: error generated by app code
...
function isResponseCodeNetworkError(code = 0 as integer) as boolean
	errorCodes = [0, -5, -6, -7, -28, -35, -52]
	return contains(errorCodes, code)
end function

function appendQueriesToUri(uri as string, queryParameters as object) as string
	if (not isEmptyString(uri) and isNonEmptyAA(queryParameters))
		first = (0 = Instr(0, uri, "?"))
		...
		for each key in queryParameters
			...
			if isNonEmptyAA(value)
				encode = getValueAtKeyPath(value, "encode", false)
				value = value.value
			end if
...
function getTransactionNetworkService() as object
	return m.global.transactionNetworkService
end function
